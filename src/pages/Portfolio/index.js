...
import { faBootstrap, faCss3Alt, faDocker, faGitAlt, faHtml5, faJs, faNodeJs, faPython, faReact, faVuejs } from "@fortawesome/free-brands-svg-icons";
import { faBriefcase, faBuilding, faCalendarDays, faCode, faEnvelope, faLaptopCode, faLocationDot } from "@fortawesome/free-solid-svg-icons";
...
const technologyIcons = {
  ...
  GitHub: faGitAlt, HTML5: faHtml5, CSS3: faCss3Alt, Bootstrap: faBootstrap,
};

const TechnologyChip = ({ skill }) => <span className="skill-chip">{technologyIcons[skill] && <Icon icon={technologyIcons[skill]} label={`${skill} icon`} />}{skill}</span>;
...
const Role = ({ role }) => (
  <Reveal className="role-reveal"><div className="role-item">
    ...
    <div className="role-meta"><span><Icon icon={faCalendarDays} label="Dates" />{role.dates}</span><span>{role.duration}</span><span><Icon icon={role.mode === "Remote" || role.mode === "Hybrid" ? faLaptopCode : faLocationDot} label="Work mode" />{role.mode}</span></div>
    {role.responsibilities.length > 0 && <ul className="role-responsibilities">{role.responsibilities.map((item) => <li key={item}>{item}</li>)}</ul>}
    <div className="role-skills"><Icon icon={faCode} label="Technologies and verified skills" /><div><strong>Technologies used</strong><div className="technology-badges">{role.technologies.map((technology) => <span key={technology}>{technology}</span>)}</div></div></div>
  </div></Reveal>
);
