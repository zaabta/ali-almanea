...
const Related = ({ article }) => {
  ...
  return <section className="related-articles" aria-labelledby="related-heading"><h2 id="related-heading">Related notes</h2><div className="related-grid">{related.map((item) => <a className="related-card" href={sitePath(`/blog/${item.slug}`)} key={item.slug}><span>{item.category}</span><h3>{item.title}</h3><small>{item.readingTime}</small></a>)}</div></section>;
};
